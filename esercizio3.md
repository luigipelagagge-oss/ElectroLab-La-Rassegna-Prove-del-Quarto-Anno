...
## 🎯 Obiettivo
Analizzare la risposta temporale **vo(t)** di un circuito **Selettore di Massimo (Analog OR)**.
...
* Tensione di soglia diodo (**Vgamma**) = 0.6 V
* **Ingresso 1 (vi1):** Onda triangolare (0V -> 5V -> 0V) con periodo T = 200 ms.
* **Ingresso 2 (vi2):** Tensione continua (0V o 5V).
...
> **vo(t) = max(vi1, vi2) - 0.6V**

### Analisi dell'Onda Triangolare
La rampa sale linearmente da 0 a 5V in 100 ms.
La pendenza (**m**) è:
> **m = 5V / 100ms = 0.05 V/ms**

---

### 1️⃣ Caso A: Ingresso 2 a Massa (vi2 = 0V)
...
**Quando si accende (ton)?**
> 0.05 * ton = 0.6  -->  **ton = 12 ms**

**Quando si spegne (toff)?**
...
* Picco massimo: 5V - 0.6V = **4.4V**

---

### 2️⃣ Caso B: Ingresso 2 Alto (vi2 = 5V)
...
| :--- | :--- | :--- | :--- |
| **Esercizio 2** | **AND** (Minimo) | Y = min(A, B) | "Pull-down" (Diodi verso Ingressi, R verso Vcc) |
